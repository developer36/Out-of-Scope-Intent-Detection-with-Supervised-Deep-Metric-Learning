...
class BERT_Loader:
    def __init__(self, args, base_attrs, logger_name='Detection'):
        ...
        self.train_examples, self.train_labeled_examples, self.train_unlabeled_examples = get_examples(args, base_attrs,
                                                                                                       'train')
    ...
    def init_loader(self, args):
        self.train_labeled_loader, _ = get_loader(self.train_labeled_examples, args, self.base_attrs['label_list'],
                                               'train_labeled', sampler_mode='random')
        self.train_unlabeled_loader, _ = get_loader(self.train_unlabeled_examples, args, self.base_attrs['label_list'],
                                                 'train_unlabeled', sampler_mode='sequential')
        self.eval_loader, _ = get_loader(self.eval_examples, args, self.base_attrs['label_list'], 'eval',
                                      sampler_mode='sequential')
        self.test_loader, self.label_map = get_loader(self.test_examples, args, self.base_attrs['label_list'], 'test',
                                      sampler_mode='sequential')
...
def get_examples(args, base_attrs, mode):
    ...
    if mode == 'train':
        ...
        for example in ori_examples:
            ...
            if (example.label in base_attrs['known_label_list']) and (
                    np.random.uniform(0, 1) <= args.labeled_ratio):  # ?<= 1
                labeled_examples.append(example)
            else:
                example.label = base_attrs['unseen_label']  # 'oos'
